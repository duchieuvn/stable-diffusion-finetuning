...
def load_config(yaml_config=None):
    ...
    return {
        ...
        "output_folder": f"../testgen/{yaml_config['folder_name']}",
...
def generate_folder_name_from_config(yaml_config, base_model=None):
    """
    Generate a descriptive folder name from the configuration.
    
    Args:
        yaml_config: Loaded YAML configuration dictionary
        base_model: Base model name (optional, for prefixing)
    ...
    # Add prefix for specific base models
    if base_model and base_model == "runwayml/stable-diffusion-v1-5":
        folder_name = f"sd15_{folder_name}"
    
    return folder_name


def get_all_lora_folders(runs_dir="../runs"):
    """
    Scan the runs directory and return all valid LoRA folders.
    
    Args:
        runs_dir: Path to the runs directory
        
    Returns:
        list: List of valid LoRA folder paths, excluding 'history' folder
    """
    ...
    # List all items in the runs directory
    for item in os.listdir(runs_dir):
        item_path = os.path.join(runs_dir, item)
        
        # Skip if it's not a directory or if it's the history folder
        if not os.path.isdir(item_path) or item == "history":
            continue
        
        # Check if this folder has LoRA adapter files
        ...
        if os.path.exists(adapter_config) and os.path.exists(adapter_model):
            lora_folders.append(item_path)
    
    return sorted(lora_folders)


def process_lora_folder(lora_folder_path, yaml_config, base_model):
    """
    Process a single LoRA folder and generate images.
    
    Args:
        lora_folder_path: Full path to the LoRA folder
        yaml_config: Loaded YAML configuration dictionary
        base_model: Base model to use for generation
    """
    # Extract folder name
    lora_folder_name = os.path.basename(lora_folder_path)
    
    print(f"\n{'='*80}")
    print(f"Processing LoRA folder: {lora_folder_name}")
    ...
    # Create base model prefix for output folder
    base_model_prefix = "sd15_" if base_model == "runwayml/stable-diffusion-v1-5" else ""
    
    # Create config for this LoRA folder
    config = {
        "base_model": base_model,
        "lora_weights_path": lora_folder_path,
        "output_folder": f"../testgen/{base_model_prefix}{lora_folder_name}",
        "default_prompt": yaml_config["default_prompt"],
        "device": "cuda" if torch.cuda.is_available() else "cpu"
    }
    
    # Make output folder path absolute
    if not os.path.isabs(config["output_folder"]):
        config["output_folder"] = os.path.join(os.path.dirname(__file__), config["output_folder"])
    ...
    if latest_subfolder:
        ...
        if compare_yaml_files(config_source, latest_config):
            ...
            folder_name = generate_folder_name_from_config(yaml_config, base_model)
...
def main():
    ...
    # Define base models to process
    base_models = [
        "SG161222/Realistic_Vision_V5.1_noVAE",
        "runwayml/stable-diffusion-v1-5"
    ]
    
    # Get all LoRA folders from runs directory
    lora_folders = get_all_lora_folders()
    
    if not lora_folders:
        print("❌ No valid LoRA folders found in runs directory")
        return
    
    print(f"\n🔍 Found {len(lora_folders)} LoRA folder(s):")
    for folder in lora_folders:
        print(f"  - {os.path.basename(folder)}")
    
    # Process each base model and LoRA folder combination
    for base_model in base_models:
        print(f"\n{'='*80}")
        print(f"Processing with base model: {base_model}")
        print(f"{'='*80}")
        
        for idx, lora_folder in enumerate(lora_folders, 1):
            try:
                process_lora_folder(lora_folder, yaml_config, base_model)
            except Exception as e:
                print(f"❌ Error processing {os.path.basename(lora_folder)}: {str(e)}")
                continue
    
    print(f"\n{'='*80}")
    print("✅ All base models and LoRA folders processed successfully!")
    print(f"{'='*80}")
